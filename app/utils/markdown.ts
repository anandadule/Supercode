...
const limitedMarkdownPlugin: Plugin = () => {
  return (tree, file) => {
    ...
    /*
     * Cast to `any` at the visit boundary to work around a nominal-type
     * conflict between the two copies of @types/unist nested under rehype-raw
     * and the top-level unist types — see markdown.ts header comment.
     */
    (visit as any)(tree, (node: UnistNode, index: number | undefined, parent: UnistParent) => {
      if (
        index == null ||
