...
/**
 * Starter framework templates shown in the empty-state and team-template picker.
 *
 * Kept in its own module (rather than in `utils/constants.ts`) so the heavy
 * GitHub repo list doesn't get bundled into every page that imports
 * `PROVIDER_LIST`. Loaded dynamically by `StarterTemplates.tsx` and
 * `TeamTemplateDialog.tsx`.
 */
...
/**
 * Lazy-loadable getter — use this from components so the static array isn't
 * pulled into the initial bundle.
 */
export async function loadStarterTemplates(): Promise<Template[]> {
  const mod = await import('./starterTemplates');
