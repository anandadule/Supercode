...
import { autoFixEnabled as autoFixStore } from '~/lib/stores/autoFix';
import { agentMode as agentModeStore } from '~/lib/stores/agentMode';
  ...
  const handleToggleFeature = useCallback(
    (id: string, enabled: boolean) => {
      switch (id) {
        ...
        case 'autoFix': {
          autoFixStore.set(enabled);
          toast.success(`Auto-fix ${enabled ? 'enabled' : 'disabled'}`);
          break;
        }

        case 'agentMode': {
          agentModeStore.set(enabled ? 'max' : 'standard');
          toast.success(`Agent mode set to ${enabled ? 'Max' : 'Standard'}`);
          break;
        }
  ...
  const features = {
    stable: [
      ...
      },
      {
        ...
        description: 'Automatically analyze and fix build/runtime errors',
        icon: 'i-ph:wrench',
        enabled: autoFixStore.get(),
        tooltip: 'When enabled, the AI will attempt to fix errors automatically',
      },
    ],
    beta: [
      {
        ...
        description: 'Use deeper reasoning for complex tasks (Max agent)',
        icon: 'i-ph:lightning-fill',
        enabled: agentModeStore.get() === 'max',
        tooltip: 'Max agent uses more tokens but provides better reasoning for complex tasks',
      },
    ],
  };
