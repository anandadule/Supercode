...
export default function LocalProvidersTab() {
  ...
  const fetchOllamaModels = async (baseUrl?: string) => {
    try {
      ...
      const ollamaUrl = baseUrl || OLLAMA_API_URL;
  ...
  const handleUpdateOllamaModel = async (modelName: string, baseUrl?: string) => {
    try {
  ...
  const handleDeleteOllamaModel = async (modelName: string, baseUrl?: string) => {
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => fetchOllamaModels(ollamaProvider?.settings.baseUrl)}
                      ...
                      <div className="grid gap-4">
                        {ollamaModels.map((model) => (
                          <ModelCard
                            key={model.name}
                            model={model}
                            onUpdate={() => handleUpdateOllamaModel(model.name, ollamaProvider?.settings.baseUrl)}
                            onDelete={() => handleDeleteOllamaModel(model.name, ollamaProvider?.settings.baseUrl)}
                          />
                        ))}
                      </div>
                    )}
