...
export default class TogetherProvider extends BaseProvider {
  ...
  async getDynamicModels(
    ...
    const isPlaceholderUrl = (url?: string) => !url || url.includes('your_') || url.includes('_here');
    const baseUrl = isPlaceholderUrl(fetchBaseUrl) ? 'https://api.together.xyz/v1' : fetchBaseUrl;
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch models from Together API: ${response.status} ${response.statusText}`);
    }
