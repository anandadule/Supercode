...
import { maybeAutoInstallForFile } from './auto-install';
import { pushBuildError } from '~/lib/stores/buildFeedback';
...
export class ActionRunner {
  ...
  async #runFileAction(action: ActionState) {
    ...
    try {
      ...
      maybeAutoInstallForFile(relativePath);
    } catch (error) {
  ...
  async #runBuildAction(action: ActionState) {
    ...
    if (exitCode !== 0) {
      ...
      // Queue the error so the chat layer can re-prompt the LLM with the failure
      pushBuildError({
