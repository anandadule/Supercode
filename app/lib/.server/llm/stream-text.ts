...
export async function streamText(props: {
  ...
  // Inject active design system prompts
  if (designSystemIds && designSystemIds.length > 0) {
    const designSystemPrompts = designSystems
      .filter((ds) => designSystemIds.includes(ds.id))
      .map((ds) => ds.promptFragment)
      .join('\n\n');

    if (designSystemPrompts) {
      systemPrompt = `${systemPrompt}
