...
const ACTION_STREAM_THROTTLE_MS = 100; // Throttle window for streaming action callbacks
const ZIP_FILENAME_HASH_LENGTH = 6; // Number of base36 chars used to uniquify the downloaded zip name
const GITHUB_VISIBILITY_PROPAGATION_DELAY_MS = 3000; // Wait after changing a GitHub repo's visibility
const GIT_REPO_INIT_DELAY_MS = 2000; // Wait for a newly created remote repo to be usable
const GIT_PUSH_MAX_ATTEMPTS = 3; // Max retry attempts when pushing files to a remote repo
const GIT_PUSH_RETRY_BASE_DELAY_MS = 2000; // Base backoff between push retries (multiplied by attempt index)
const GITLAB_BRANCH_CREATE_DELAY_MS = 1000; // Wait after creating a GitLab branch before writing to it
...
export class WorkbenchStore {
  ...
  /** Callback for broadcasting file changes during collaboration */
  #fileChangeBroadcast: ((path: string, content: string) => void) | null = null;
  constructor() {
    if (import.meta.hot) {
  ...
  async saveFile(filePath: string) {
    ...
    // Broadcast file change if collaboration is active
    if (this.#fileChangeBroadcast) {
      this.#fileChangeBroadcast(filePath, document.value);
    }
  ...
  /**
   * Set a callback to broadcast file changes to other collaborators
   */
  setFileChangeBroadcast(cb: ((path: string, content: string) => void) | null) {
    this.#fileChangeBroadcast = cb;
  }
  ...
  async downloadZip() {
    ...
    const timestampHash = Date.now().toString(36).slice(-ZIP_FILENAME_HASH_LENGTH);
    ...
    try {
      ...
      if (isGitHub) {
        ...
        try {
          ...
          if (repo.private !== isPrivate) {
            ...
            try {
              ...
              await new Promise((resolve) => setTimeout(resolve, GITHUB_VISIBILITY_PROPAGATION_DELAY_MS));
          ...
          if (error instanceof Error && 'status' in error && error.status === 404) {
            ...
            await new Promise((resolve) => setTimeout(resolve, GIT_REPO_INIT_DELAY_MS));
          } else {
        ...
        const pushFilesToRepo = async (attempt = 1): Promise<string> => {
          const maxAttempts = GIT_PUSH_MAX_ATTEMPTS;
          ...
          try {
            ...
            if ((visibilityJustChanged || attempt === 1) && attempt < maxAttempts) {
              const delayMs = attempt * GIT_PUSH_RETRY_BASE_DELAY_MS;
      ...
      if (isGitLab) {
        ...
        if (!repo) {
          ...
          await new Promise((r) => setTimeout(r, GIT_REPO_INIT_DELAY_MS));
        }
        ...
        if (!branchRes || !branchRes.ok) {
          ...
          await new Promise((r) => setTimeout(r, GITLAB_BRANCH_CREATE_DELAY_MS));
        }
